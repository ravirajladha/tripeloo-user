...
import TourAndPackageSearchForm from "./TourAndPackageSearchForm";

export type SearchRealEstateTab = "Buy" | "Rent" | "Sell";

export interface HeroTourAndPackageSearchFormProps {
  className?: string;
  currentTab?: SearchRealEstateTab;
}

const HeroTourAndPackageSearchForm: FC<HeroTourAndPackageSearchFormProps> = ({
  className = "",
  currentTab = "Buy",
}) => {
  ...
  const renderForm = () => {
    switch (tabActive) {
      case "Buy":
        return <TourAndPackageSearchForm />;
  ...
  return (
    <div
      className={`nc-HeroTourAndPackageSearchForm w-full max-w-6xl py-5 lg:py-0 ${className}`}
    >
      {renderTab()}
      {renderForm()}
    </div>
...
export default HeroTourAndPackageSearchForm;
