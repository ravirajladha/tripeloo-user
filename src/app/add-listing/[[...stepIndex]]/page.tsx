...
const Page = ({
  ...
  params: { stepIndex: string[] }; // stepIndex should be an array to match the [[...stepIndex]] route
  searchParams?: { [key: string]: string | string[] | undefined };
}) => {
  let ContentComponent = PageAddListing1;

  switch (Number(params.stepIndex[0])) { // Access the first element of the array
    case 1:
...
// Define generateStaticParams to inform Next.js about paths to pre-render
export async function generateStaticParams() {
  // Define the steps you want to generate static paths for
  const steps = Array.from({ length: 10 }, (_, i) => i + 1); // Generate [1, 2, ..., 10]

  // Map over the steps to create an array of paths
  return steps.map((step) => ({
    stepIndex: [`${step}`], // Convert each step to an array format
  }));
}
